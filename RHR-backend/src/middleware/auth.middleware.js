...
const authenticate = async (req, res, next) => {
  try {
    ...
    // Salesmen and drivers live in their own tables now — pick the right
    // one based on the role the token was issued with.
    ...
    // salesmen/drivers have no `role` column — the table itself is the discriminator
    ...
    if (!user.is_approved && (user.role === 'customer' || user.role === 'salesman' || user.role === 'driver')) {
